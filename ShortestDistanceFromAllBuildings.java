class Solution {
  ...
  private int[][] directions = {{1,0},{-1,0},{0,1},{0,-1}};
    ...
    public int shortestDistance(int[][] grid) {
        ...
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
                if (grid[i][j] == 1) {
                    totalBuildings++;
                    if (!traverse(grid, i, j, dist, canReach)) return -1;
                }
            }
        }
    ...
    private boolean traverse(int[][] grid, int row, int col, int[][] dist, int[][] canReach) {
        int rows = grid.length, cols = grid[0].length;
        ...
        while(!q.isEmpty()) {
            ...
            for(int i=0;i<size;i++) {
                ...
                for(int k=0;k<4;k++) {
                    int rr = directions[k][0]+r;
                    int cc = directions[k][1]+c;
